...
- **Tailored Cover Letter** — a second artifact alongside the Tailored Resume, grounded in Candidate facts plus a "why this company" section. Needs its own dedicated conversational flow to do well; not a small addition. A related idea worth keeping so it isn't lost: an Email template (the message that accompanies a submitted application) as a sibling artifact, with both the Cover Letter and the Email template scoped **per-Profile** — each Profile keeps its own default tailoring baseline for both, the same way each Profile already keeps its own resume content. Undecided; needs its own domain-modeling and scoping pass alongside the conversational flow above.
